...
OUT_DIR = Path("/Users/christian/Desktop/HemaFrag/artifacts/genemapper_like_100file_eval_2025")
...
def _find_peaks_common(values: np.ndarray, ladder: str, min_width: tuple[float, float] | None = None) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    if ladder == "LIZ":
...
def detect_width_prom_halfwidth(values: np.ndarray, ladder: str) -> np.ndarray:
    ...
    return np.asarray([int(p) for p, wd in zip(peaks, widths) if wd >= min_half], dtype=int)
...
def detect_wavelet(values: np.ndarray, ladder: str) -> np.ndarray:
    ...
    if peaks.size == 0:
        return peaks
    vals = values[peaks]
    return np.asarray(peaks[vals >= max(15.0, float(np.percentile(values, 85)) * (0.08 if ladder == "LIZ" else 0.07))], dtype=int)


def detect_wavelet_slope(values: np.ndarray, ladder: str) -> np.ndarray:
    peaks = detect_wavelet(values, ladder)
    if peaks.size == 0:
        return peaks
    grad = np.gradient(values)
    keep = []
    rise_thr = 2.0 if ladder == "LIZ" else 1.5
    fall_thr = -2.0 if ladder == "LIZ" else -1.5
    window = 12 if ladder == "LIZ" else 10
    for peak in peaks:
        start = max(0, int(peak) - window)
        end = min(len(values), int(peak) + window + 1)
        left_rise = float(np.max(grad[start:int(peak)])) if int(peak) > start else 0.0
        right_fall = float(np.min(grad[int(peak):end])) if end > int(peak) else 0.0
        if left_rise >= rise_thr and right_fall <= fall_thr:
            keep.append(int(peak))
    return np.asarray(keep, dtype=int)


def detect_savgol_11_3(values: np.ndarray, ladder: str) -> np.ndarray:
    window = 11 if len(values) >= 11 else max(5, len(values) // 2 * 2 + 1)
    smooth = signal.savgol_filter(values, window_length=window, polyorder=3, mode="interp")
    return detect_width_prom_loose(np.asarray(smooth, dtype=float), ladder)


def detect_savgol_17_3(values: np.ndarray, ladder: str) -> np.ndarray:
    window = 17 if len(values) >= 17 else max(5, len(values) // 2 * 2 + 1)
    smooth = signal.savgol_filter(values, window_length=window, polyorder=3, mode="interp")
    return detect_width_prom_loose(np.asarray(smooth, dtype=float), ladder)


def detect_wavelet_local_gap(values: np.ndarray, ladder: str) -> np.ndarray:
    peaks = detect_wavelet(values, ladder)
    if peaks.size <= 2:
        return peaks
    # local-southern-inspired proxy: remove dense local clusters that imply implausibly tiny adjacent scan gaps
    min_gap = 18 if ladder == "LIZ" else 14
    keep = [int(peaks[0])]
    for i in range(1, len(peaks) - 1):
        left_gap = int(peaks[i] - peaks[i - 1])
        right_gap = int(peaks[i + 1] - peaks[i])
        if left_gap < min_gap and right_gap < min_gap:
            continue
        keep.append(int(peaks[i]))
    keep.append(int(peaks[-1]))
...
DETECTORS = {
    ...
    "wavelet_slope": detect_wavelet_slope,
    "savgol_11_3": detect_savgol_11_3,
    "savgol_17_3": detect_savgol_17_3,
    "wavelet_local_gap": detect_wavelet_local_gap,
}
...
def rust_selected_map(paths: list[Path]) -> dict[str, list[int]]:
    ...
    for path in paths:
        resp = worker.request(path, "clonality", timeout)
...
def main() -> None:
    ...
    for _, row in chosen.iterrows():
        ...
        for bname, bfn in BASELINES.items():
            ...
            for dname, dfn in DETECTORS.items():
                ...
                head4, tail4 = head_tail_recall(cand, selected, 4, 12)
                rows.append(
                    {
                        "file": row["raw_file"],
                        "month": row["Month"],
    ...
    agg = (
        out_df.groupby(["ladder_type", "bucket", "baseline", "detector"], as_index=False)[
            ["candidate_count", "early_blob_count", "recall_tol12", "head4_recall", "tail4_recall"]
        ]
        .mean()
        .sort_values(
            ["ladder_type", "bucket", "recall_tol12", "head4_recall", "early_blob_count"],

...
class TabFlt3Validation(QWidget):
    ...
    def _build_settings_card(self) -> QWidget:
        ...
        rust_note = QLabel(
            "FLT3 archive validation follows the global setting in Settings: "
    ...
    def set_analysis(self, analysis_id: str) -> None:
        ...
        if enabled and not self._flt3_archive_support_available():
            self._set_workflow_status(self._flt3_archive_support_message(), "disabled")
    ...
    def _refresh_command_preview(self) -> None:
        if not self._flt3_archive_support_available():
            self.command_preview.setPlainText(self._flt3_archive_support_message())
            return
        lines = [
        ...
        if bool(APP_SETTINGS.get("engine", {}).get("use_rust", True)):
            lines.append("  --use-rust")
        else:
            lines.append("  # Rust engine disabled in Settings; validation will use legacy Python ladder fitting")
    ...
    def _refresh_action_buttons(self) -> None:
        support = self._flt3_archive_support_available()
        self.btn_run.setEnabled(support and self._active_worker is None and self.isEnabled())
        self.btn_open_run.setEnabled(
            support and self._current_run_dir is not None and self._current_run_dir.exists()
        )
        self.btn_open_workbook.setEnabled(
            support
            and self._current_workbook_path is not None
            and self._current_workbook_path.exists()
        )

    def _set_busy(self, busy: bool) -> None:
        self.btn_run.setEnabled(
            self._flt3_archive_support_available() and (not busy) and self.isEnabled()
        )
        self.btn_open_run.setEnabled(
            self._flt3_archive_support_available()
            and (not busy)
    ...
    def on_run_validation(self) -> None:
        if not self._flt3_archive_support_available():
            QMessageBox.warning(self, "FLT3 Archive Runner", self._flt3_archive_support_message())
            return
        try:

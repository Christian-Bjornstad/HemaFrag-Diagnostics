...
- 2026-07-28: Python analysis paths reuse a versioned per-process FSA artifact, and all batch pools must use `core.concurrency` so Python workers do not multiply unrestricted Rust/numeric threads. Real A/B evidence showed exact parity, one decode instead of three, and `5.42%` median/`13.88%` p95 improvement; the FLT3 25-file worker matrix favored eight workers on the 16-logical-CPU reference machine.
- 2026-07-28: Analysis entries, HTML reports, and clonality/FLT3 tracking rows retain full source SHA-256, ladder engine/strategy/reason codes, app version, and consumed manual-adjustment hash. SQLite run-ledger support is a validated append/idempotency/round-trip prototype only; Excel remains production storage until real workbook parity and migration gates pass.
- 2026-07-28: Ladder Studio rerun success is hash-specific: review bundles and child run manifests record whether the exact saved correction was consumed, while candidate rank/margin/threshold evidence stays shadow-only. Tracking workbooks expose advisory QC trends, but Shewhart/EWMA signals require at least 20 explicitly selected baseline runs and never change runtime thresholds.
- 2026-07-28: General analysis profiles use `hemafrag_general_profile_v1` with version, validation status, ladder steps, size-standard/trace channels, bp range, report fields, and fingerprint. Plan 13 engineering gates pass, but all clinical algorithm promotion remains blocked until FLT3 area-bias evidence, reviewed interpretation comparison, and chemist shadow approval exist.
...
- Clonality chemist labeling shows same-DIT same-assay parallels together for visual comparison, but labels are per injection because one parallel can be QC fail while the other is interpretable. Current chemist labels are `monoklonal`, `monoklonal_pa_poly`, `polyklonal`, `oligoklonal`, `irregulaer`, `lite_pcr_produkt`, `intet_pcr_produkt`, `qc_teknisk_fail`, and `usikker_review`; old `bi_oligoklonal` and `intet_pcr_produkt_darlig_dna` normalize forward. IKZF1 and Ktr-albumin are excluded from current ML labeling/training batches.
- Clonality labeling plot x-axis start overrides: IGK 100 bp, TCRbA/TCRbB 210 bp, TCRbC 150 bp, TCRgA 110 bp, TCRgB 60 bp, DHJH_E 60 bp, KDE 200 bp. Rerun groups are paginated to max two visible plots at a time.
- First completed clonality ML labeling batch produced 526 merged chemist labels across 12 assays. Strict production readiness is still not met; first RF/ExtraTrees run is candidate-only research evidence, not report/runtime eligible. Next labeling should prioritize rare labels and additional independent DIT/source-run support.
